...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        if self.config['TRACKERS_TO_EVAL'] is None:
            ...
            print('No trackers specified, evaluating all in folder: ' + self.tracker_fol)
        else:
            self.tracker_list = self.config['TRACKERS_TO_EVAL']
            print('Evaluating the following trackers: ' + ', '.join(self.tracker_list))

        if self.config['TRACKER_DISPLAY_NAMES'] is None:
            self.tracker_to_disp = dict(zip(self.tracker_list, self.tracker_list))
            print('No tracker display names specified, using tracker filenames.')
        elif (self.config['TRACKERS_TO_EVAL'] is not None) and (
                len(self.config['TRACKER_DISPLAY_NAMES']) == len(self.tracker_list)):
            self.tracker_to_disp = dict(zip(self.tracker_list, self.config['TRACKER_DISPLAY_NAMES']))
            print('Using specified tracker display names.')
        else:
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    print("cur file",curr_file)
                    print("the cur os path is",os.path.isfile(curr_file))
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
